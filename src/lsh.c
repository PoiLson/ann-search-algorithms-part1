...
int hash_func_impl_lsh(const void* p, const LSH* lsh, int table_index, int* ID)
{
    ...
    for (int i = 0; i < lsh->k; i++)
    {
        ...
        long long term = (r_i * h_i) % lsh->num_of_buckets;
        
        // Ensure term is positive before adding
        if (term < 0)
            term += lsh->num_of_buckets;
            
        id = (id + term) % lsh->num_of_buckets;
    }

    *ID = (int)id;
    int bucket_idx = (*ID) % lsh->table_size;
...
LSH* lsh_init(const struct SearchParams* params, const struct Dataset* dataset)
{
    ...
    lsh->table_size = nearest_prime(dataset->size / 4); // e.g., ~3000
    lsh->num_of_buckets = nearest_prime(1 << 16); // e.g., ~6000
