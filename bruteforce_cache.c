...
BruteForceCache* bruteforce_compute(const Dataset* dataset, const Dataset* query_set, int N)
{
    ...
    for (int q_idx = 0; q_idx < query_set->size; q_idx++) {
        ...
        // Start timing for this query (use wall-clock time for parallel execution)
        ...
        for (int i = 0; i < dataset->size; i++) {
            all_dists[i] = euclidean_distance(query, dataset->data[i], 
                                             dataset->dimension, 
                                             query_set->data_type, 
                                             dataset->data_type);
            all_indices[i] = i;
        ...
        // Record time taken for this query (wall-clock time)
...
bool bruteforce_cache_save(const BruteForceCache *cache, const char *cache_path) {
    if (!cache || !cache_path) return false;
...
void bruteforce_cache_free(BruteForceCache *cache) {
    ...
    if (cache->neighbors) {
        for (int q = 0; q < cache->n_queries; q++) {
            if (cache->neighbors[q]) free(cache->neighbors[q]);
        }
    ...
    if (cache->distances) {
        for (int q = 0; q < cache->n_queries; q++) {
            if (cache->distances[q]) free(cache->distances[q]);
        }
...
char* bruteforce_cache_get_path(const char *dataset_path, const char *query_path, int N) {
    if (!dataset_path || !query_path) return NULL;
    ...
    snprintf(cache_path, 512, "Data/.cache/%s_%s_N%d_bruteforce.cache", 
             dataset_name, query_name, N);
