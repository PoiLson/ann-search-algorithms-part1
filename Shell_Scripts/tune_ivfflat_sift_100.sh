...
# IVFFlat Parameter Tuning Script (SIFT, 100 queries)
# Grid: kclusters={10,20,30,40,50,60,80}, nprobe={2,3,4,5}

OUT_DIR="runs/ivfflat_sift_100"
mkdir -p "$OUT_DIR"
CSV="$OUT_DIR/ivfflat_sift_100_grid.csv"
: > "$CSV"
echo "KCLUSTERS,NPROBE,AvgRecall,AvgAF,AvgTApprox,AvgTTrue,QPS" >> "$CSV"

KCLUSTERS=(10 20 30 40 50 60 80)
NPROBES=(2 3 4 5)
...
for kclusters in "${KCLUSTERS[@]}"; do
  for nprobe in "${NPROBES[@]}"; do
    echo "Running IVFFlat (SIFT-100): kclusters=$kclusters nprobe=$nprobe" >&2
    tmpf=$(mktemp)
    if ! ./search -d "$DATASET" -q "$QUERIES" -ivfflat -kclusters "$kclusters" -nprobe "$nprobe" -o "$tmpf" -N 1 -R 50000 -type sift -range false >/dev/null 2>&1; then
