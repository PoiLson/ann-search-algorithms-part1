...
# IVFPQ Parameter Tuning Script (MNIST, 100 queries)
# Grid: kclusters={10,20,50,60}, nprobe={5,20,40}, nbits={8,16}, M={16,28,56,98}

OUT_DIR="runs/ivfpq_mnist_100"
mkdir -p "$OUT_DIR"
CSV="$OUT_DIR/ivfpq_mnist_100_grid.csv"
: > "$CSV"
  ...
  for nprobe in "${NPROBES[@]}"; do
    for M in "${MBLOCKS[@]}"; do
      for nbits in "${NBITS_LIST[@]}"; do
        echo "Running IVFPQ (MNIST-100): kclusters=$kclusters nprobe=$nprobe M=$M nbits=$nbits" >&2
        tmpf=$(mktemp)
        if ! ./search -d "$DATASET" -q "$QUERIES" -ivfpq -kclusters "$kclusters" -nprobe "$nprobe" -M "$M" -nbits "$nbits" -o "$tmpf" -N 1 -R 50000 -type mnist -range false -seed 42 >/dev/null 2>&1; then
